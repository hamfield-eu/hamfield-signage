...
| **User locked out by two-factor auth**       | Lost authenticator _and_ recovery codes. There is no email channel, by design — the server is the only way back                                                  | `dc exec api node apps/api/dist/cli/disable-mfa.js <email>` `[PROD]`. Clears MFA only; the password is untouched. Confirm identity out-of-band first |
...
| Turning off a user's two-factor auth (`disable-mfa`)      | Owner. **Confirm the person's identity out-of-band first** — this command is the entire second factor for that account, so running it on a request that arrived by email or chat defeats the point of having it |
