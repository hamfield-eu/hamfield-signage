...
> If your `web` image predates 2026-09-09, add `dc restart web` after every
> `up -d`. Older builds pin the `api` container's IP at nginx startup and 502 on
> all `/api/…` calls once it changes — see [§14](#14-troubleshooting). Rebuilding
> `web` once from current `infra/docker/web-nginx.conf` removes the need.
...
| **502 on every `/api/…` right after `up -d`**   | Stale upstream DNS in `web`. nginx resolved `api` once at startup and cached the IP; the recreated `api` container has a new one, and Docker may already have handed the old address to a different container — which refuses :4000 outright, so it fails instantly rather than timing out. Immediate fix: `dc restart web`. Permanent fix: rebuild `web` from a checkout that has the `resolver` + `set $api_upstream` form in `infra/docker/web-nginx.conf` (added 2026-09-09 after this hit production). |
